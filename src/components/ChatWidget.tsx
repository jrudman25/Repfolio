...
export default function ChatWidget() {
  ...
  return (
    <>
      ...
      {isOpen && <div id="portfolio-chat" role="region" aria-labelledby="chat-title" onKeyDown={e => { if (e.key === 'Escape') { e.preventDefault(); closeChat() } }} className={`fixed bottom-6 right-6 w-96 h-[500px] bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col z-50 transition-all duration-300 origin-bottom-right ${isOpen ? 'scale-100 opacity-100' : 'scale-0 opacity-0 pointer-events-none'}`}>
        {/* Header */}
        ...
        {/* Messages */}
        <div role="log" aria-label="Chat messages" aria-live="polite" className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="text-center text-zinc-500 mt-10">
              <Bot className="w-12 h-12 mx-auto mb-3 opacity-20" />
              <p>Ask questions about your indexed repository READMEs. Answers use README content only.</p>
            </div>
          )}
        ...
        {/* Input */}
        <form aria-label="Send chat message" onSubmit={handleSend} className="p-3 border-t border-zinc-800 bg-zinc-950 rounded-b-2xl flex gap-2">
          <input
            ref={inputRef}
